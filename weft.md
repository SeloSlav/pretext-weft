...
- Keep the preset topology and behavior model when the surface is still fundamentally grass-like, wall-like, rock-like, fire-like, or sky-like.
- Prefer spreading and overriding shipped defaults such as `DEFAULT_GRASS_FIELD_PARAMS` or `DEFAULT_FISH_SCALE_PARAMS` instead of rewriting large param objects from scratch.
